                ...
                @else
                    <div class="space-y-4">
                        @foreach ($orders as $order)
                            <div
                                class="bg-white border border-gray-100 rounded-lg p-6 hover:border-gray-200 transition-colors duration-200">
                                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                    <!-- Order Info -->
                                    <div>
                                        <div class="flex items-center gap-2 mb-2">
                                            <i class="bi bi-receipt text-custom"></i>
                                            <h6 class="font-semibold text-gray-900">Order
                                                #{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}</h6>
                                        </div>
                                        ...
                                        <div class="flex items-center text-sm text-gray-500">
                                            ...
                                            <span>Batas waktu:
                                                {{ $order->created_at->addDays(1)->format('d M Y H:i') }}</span>
                                        </div>
                                    </div>
